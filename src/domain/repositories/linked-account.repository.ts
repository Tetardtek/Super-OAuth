...
export interface ILinkedAccountRepository {
  ...
  findByProviderAccount(provider: OAuthProvider, providerId: string, tenantId: string): Promise<LinkedAccount | null>;
  ...
  existsByProviderAccount(provider: OAuthProvider, providerId: string, tenantId: string): Promise<boolean>;
}
